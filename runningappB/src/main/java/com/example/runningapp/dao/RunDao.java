...
public class RunDao {
 ...
 // this method is where a run is added to the database
    public int addRun(Run run) {
        String sql = "INSERT INTO runsappb (userID, runDate, runDistance, runDuration, runSpeed, unit, additionalDetails, isDeleted) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
 ...
 // this method will update an existing run's details in the database
    public int updateRun(Run run) {
        String sql = "UPDATE runsappb SET userID = ?, runDate = ?, runDistance = ?, runDuration = ?, runSpeed = ?, unit = ?, " +
 ...
 // this method gets all non-deleted runs for a specific user, sorted by date
    public List<Run> getRunsByUserId(int userId) {
    	String sql = "SELECT * FROM runsappb WHERE userID = ? AND isDeleted = false ORDER BY runDate DESC";
